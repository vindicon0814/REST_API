...
const LABORATORY_DIR = "/LABORATORY/:LABORATORY";
...
app.get("/pushxpert/:SYSTEM_KEY"
		...
		+ PERFORMED_BY_DIR
		, (req, res) => {
	console.log("Accessing GX Alert with System Key: " + req.params.SYSTEM_KEY);
	...
	const LABORATORY_REQ = req.params.LABORATORY
	if(LABORATORY_REQ == null || LABORATORY_REQ.toUpperCase() == 'NULL'){ var LABORATORY_VALUE = '' }
	else{ var LABORATORY_VALUE = req.params.LABORATORY }
	...
	connection_to_itisdb_xpert.query(queryString, [SYSTEM_KEY_REQ], (err, rows, fields) => {
		...
		if (system_key_num == 1){
			...
			if(CASE_NUMBER_REQ == null || CASE_NUMBER_REQ.toUpperCase() == 'NULL' || SPECIMEN_CODE_REQ == null || SPECIMEN_CODE_REQ.toUpperCase() == 'NULL'){
				...
				connection_to_itisdb_xpert.query(queryString, [
					CASE_NUMBER_REQ,
