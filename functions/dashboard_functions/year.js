// YEAR
app.get("/dashboard/:system_key/year/:year", (req, res) => {
	...
	connection_to_itisdb.query(queryString, [systemKey], (err, rows, fields) => {
		...
		if (system_key_num == 1){
			...
			const queryString = "SELECT * FROM dashboard WHERE year = ?"
			connection_to_itis_dashboard.query(queryString, [yearReq], (err, rows, fields) => {
