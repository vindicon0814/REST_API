// Complete - YEAR - INDICATOR ID
app.get("/dashboard/:system_key/year/:year/indicator_id/:indicator_id", (req, res) => {
	...
	connection_to_itisdb.query(queryString, [systemKey], (err, rows, fields) => {
		...
		if (system_key_num == 1){
			...
			const queryString = "SELECT * FROM dashboard WHERE year = ? AND indicator_id = ?"
			connection_to_itis_dashboard.query(queryString, [yearReq,indicatorIDReq], (err, rows, fields) => {
