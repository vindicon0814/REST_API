// MONTH
app.get("/dashboard/:system_key/month/:month", (req, res) => {
	...
	connection_to_itisdb.query(queryString, [systemKey], (err, rows, fields) => {
		...
		if (system_key_num == 1){
			...
			const queryString = "SELECT * FROM dashboard WHERE month = ?"
			connection_to_itis_dashboard.query(queryString, [monthReq], (err, rows, fields) => {
