...
app.get("/hfacility/:system_key/parent_facility/:parent_facility/current/:currentVal/offset/:offsetVal", (req, res) => {
	...
	connection_to_itisdb.query(queryString, [systemKey], (err, rows, fields) => {
		...
		if (system_key_num == 1){
			...
			if (parentFacilityReq == 1){
				...
				var queryString = "SELECT * FROM hfacility ORDER BY name ASC LIMIT ?, ?"
				
			}else if (parentFacilityReq == 0){
				
				var queryString = "SELECT * FROM hfacility WHERE parent_facility IS NULL ORDER BY name ASC LIMIT ?, ?"
...
eval(fs.readFileSync('functions/hfacility_functions/all_hfacility.js')+'');
