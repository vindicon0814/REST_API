            ...
            connection_to_itisdb.query(queryString, [SYSTEM_KEY_REQ], (err, rows, fields) => {
                ...
                if (system_key_num == 1) {
                    ...
                    if (CASE_NUMBER_REQ == null || CASE_NUMBER_REQ.toUpperCase() == 'NULL' || LAST_NAME_REQ == null || LAST_NAME_REQ.toUpperCase() == 'NULL') {
                        ...
                        const Patient_dataquery1 = "SELECT patient_data_pending.patientID AS patientID, patient_data_pending.birthDate AS birthDate, patient_data_pending.birthPlace AS birthPlace, patient_data_pending.gender AS gender, patient_data_pending.civilStatus AS civilStatus, patient_data_pending.bloodType AS bloodType, patient_data_pending.landline AS landline, patient_data_pending.mobile AS mobile, patient_data_pending.email AS email, patient_data_pending.religion AS relegion, patient_data_pending.occupation AS occupation, patient_data_pending.address AS address, patient_data_pending.gLocation AS glocation, patient_data_pending.zipCode AS zipCode, patient_data_pending.lastName AS lastName, patient_data_pending.firstName AS firstName, patient_data_pending.middleName AS middleName, patient_data_pending.name_suffix AS name_suffix, patient_data_pending.philHealth AS philHealth, patient_data_pending.nationality AS nationality, "
                        ...
                        var queryFromDB = "FROM patient_data_pending ";

                        //var queryInnerJoin1 = "INNER JOIN patient ON patient.patientID = patient_data_pending.patientID ";

                        var queryInnerJoin2 = "INNER JOIN tb_case ON tb_case.patientID = patient_data_pending.patientID ";

                        var queryWhere1 = "WHERE tb_case.registrationCode = ? AND patient_data_pending.lastName = ? ";
                        
                        var Patient_dataquery = Patient_dataquery1 + Patient_dataquery2 + Patient_dataquery3 + Patient_dataquery4 + Patient_dataquery5 + Patient_dataquery6 + queryFromDB + queryInnerJoin2 + queryWhere1;
                        ...
                        connection_to_itisdb.query(Patient_dataquery, [CASE_NUMBER_REQ, LAST_NAME_REQ], (err, rows, fields) => {
                            ...
                            if (count_patient_rows == 1) {
                                ...
                                const queryString = "UPDATE patient_data_pending INNER JOIN tb_case ON tb_case.patientID = patient_data_pending.patientID SET "
                                        + "birthDate = ?,"
                                        + "birthPlace = ?,"
                                        + "gender = ?,"
                                        + "civilStatus = ?,"
                                        ...
                                        + "nationality = ?,"
                                        + "last_update = ?"
                                    + " WHERE tb_case.registrationCode = ? AND patient_data_pending.lastName = ?"
